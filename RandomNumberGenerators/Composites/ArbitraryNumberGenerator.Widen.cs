...
partial class ArbitraryNumberGenerator {
  ...
  /// </example>
  /// <summary>
  ///   Fills the specified span with random bytes. More efficient than the array-returning
  ///   <see cref="ConcatGenerator(int)"/> when the caller owns the buffer.
  /// </summary>
  public unsafe void NextBytes(Span<byte> buffer) {
    ...
    fixed (byte* pointer = buffer) {
      ...
      var random = rng.Next();
      switch (count) {
        case 1: *index = (byte)random; break;
        case 2: *(ushort*)index = (ushort)random; break;
        case 3:
          *(ushort*)index = (ushort)random;
          index[2] = (byte)(random >> 16);
          break;
        case 4: *(uint*)index = (uint)random; break;
        case 5:
          *(uint*)index = (uint)random;
          index[4] = (byte)(random >> 32);
          break;
  ...
  /// <summary>
  ///   Generates a random RFC 4122 version-4 (random) <see cref="Guid"/>.
  /// </summary>
  ...
  /// <summary>
  ///   In-place Fisher-Yates shuffle of the given list.
  /// </summary>
  public void Shuffle<T>(IList<T> list) {
    ArgumentNullException.ThrowIfNull(list);
    for (var i = list.Count - 1; i > 0; --i) {
      var j = (int)this.ModuloRejectionSampling((ulong)(i + 1));
      (list[i], list[j]) = (list[j], list[i]);
    }
  }

  /// <summary>
  ///   Returns one uniformly random element from the given list.
  /// </summary>
  public T Choice<T>(IReadOnlyList<T> source) {
    ArgumentNullException.ThrowIfNull(source);
    ArgumentOutOfRangeException.ThrowIfZero(source.Count, nameof(source));
    return source[(int)this.ModuloRejectionSampling((ulong)source.Count)];
  }

  /// <summary>
  ///   Returns <paramref name="count"/> uniformly random elements from <paramref name="source"/> without replacement.
  ///   Uses Algorithm R (reservoir sampling) for O(count) memory.
  /// </summary>
  public T[] Sample<T>(IReadOnlyList<T> source, int count) {
    ArgumentNullException.ThrowIfNull(source);
